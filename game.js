(() => {
  ...
  let images=[],game,aim=220,current=0,next=1,ready=false,paused=false,best=0,particles=[],popups=[],last=0,accumulator=0,toastUntil=0,audio=null,sound=false,dragging=null;
  ...
  function reset(){game.reset();current=randomFruit();next=randomFruit();aim=220;particles=[];popups=[];$('score').textContent='0';$('end').hidden=true;$('confirm').hidden=true;$('hint').classList.remove('used');$('status').textContent='相同水果碰一碰，变大一点点';paused=false;updateNext();canvas.focus({preventScroll:true});}
  function drop(){if(!ready||paused||game.over)return;if(game.drop(current,aim)){beep(0);current=next;next=randomFruit();updateNext();$('hint').classList.add('used');}}
  ...
  function frame(now){if(!last)last=now;const elapsed=Math.min(now-last,50);last=now;if(ready&&!paused&&!document.hidden){accumulator+=elapsed;while(accumulator>=1000/120){game.step(1000/120);accumulator-=1000/120;}}else accumulator=0;render(now);requestAnimationFrame(frame);}
